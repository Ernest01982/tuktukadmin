...
type Row = { id: string; full_name: string | null; phone: string | null; email: string | null; is_active: boolean };
...
export default function Riders() {
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-semibold mb-4">Riders</h1>
      {err && <div className="text-red-600">{err}</div>}
      <div className="bg-white border rounded overflow-auto">
        <table className="min-w-full text-sm">
          <thead className="bg-gray-50"><tr><Th>ID</Th><Th>Name</Th><Th>Phone</Th><Th>Email</Th><Th>Active</Th></tr></thead>
          <tbody>
            {rows.map(r => (
              <tr key={r.id} className="border-t">
                <Td mono>{r.id}</Td><Td>{r.full_name ?? '-'}</Td><Td>{r.phone ?? '-'}</Td>
                <Td>{r.email ?? '-'}</Td><Td>{r.is_active ? 'Yes' : 'No'}</Td>
              </tr>
            ))}
          </tbody>
        </table>
